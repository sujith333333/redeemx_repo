...
import { getAllPoints } from "../../api/user";
...
const UserHomeScreen = () => {
  ...
  const barData = {
    labels: ["Available Points", "Credited Points", "Debited Points"],
    datasets: [
      {
      ...
      <Row className="justify-content-center mb-3">
        <Col xs={12} md={8} lg={6}>
          <Card.Body className="d-flex justify-content-between align-items-center">
            {[{ type: "balance", label: "Available Points", color: "#1E3A8A" }, { type: "credited", label: "Credited Points", color: "#047857" }, { type: "debited", label: "Debited Points", color: "#B91C1C" }].map((btn, index) => (
              <Button
                key={index}
